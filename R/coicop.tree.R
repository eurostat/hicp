...
# derive coicop tree:
tree <- function(id, by=NULL, w=NULL, w.tol=1/100, max.lvl=NULL, unbundle=TRUE){

  # @Args:
  # id        character vector of coicop ids
  # by        vector specifying the variable to be used for
  #           merging the tree, e.g. vector of dates for merging over
  #           time or a vector of countries for merging across space
  # w         numeric coicop weight. if supplied, it is checked that
  #           the weight of children approx. add up to the corresponding
  #           weight of the parent. default is NULL
  # w.tol     numeric tolerance for checking against weights. only
  #           relevant when w is not null. note that w.tol is multiplied
  #           by the number of detected coicop children
  # unbundle  logical indicating if coicop bundles (e.g.
  #           08X, 0531_2) should be taken into account or not
  # max.lvl   single integer specifying the maximum depth or deepest
  #           coicop level allowed. if NULL the deepest level found
  #           in 'id' is used
  ...
  if(all(nas)){
    ...
    lvl <- hicp::level(id=id, unbundle=TRUE)
    ...
    p <- hicp::parent(id=id, flag=FALSE, unbundle=unbundle, direct=FALSE)
