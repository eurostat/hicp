...
# # derive coicop parents:
# parents <- function(id, unbundle=TRUE, direct.only=FALSE){
#
#   # @Args:
#   # id:           character vector of coicop ids
#   # unbundle      logical indicating if coicop bundles (e.g.
#   #               08X, 0531_2) should be taken into account or not
#   # direct.only:  logical indicating if only direct children
#   #               should be flagged TRUE (e.g. 03->031) or
#   #               also lower-level childs (e.g. 03->0311) if
#   #               the childs in between are missing
#
...
#   .check.char(x=id)
#   .check.log(x=unbundle, min.len=1, max.len=1, na.ok=FALSE)
#   .check.log(x=direct.only, min.len=1, max.len=1, na.ok=FALSE)
#
#   # set to NA if no valid coicop code:
#   id[!hicp::is.coicop(id, unbundle=unbundle)] <- NA_character_
#
...
#     n <- nchar(id.tab)
#
#     # loop over coicops ids:
#     out <- rep(NA_character_, length(id.tab))
#     if(direct.only){
#       for(i in seq_along(id.tab)){
#         id.sub <- id.tab[n<nchar(id.tab[i])]
...
#     # set all-items hicp as parent of divisions:
#     if(any(id.tab=="00", na.rm=TRUE)){
#       out[id.tab!="00" & grepl(pattern="^\\d{2}$", x=id.tab)] <- "00"
#       if(!direct.only){
#         out[id.tab!="00" & is.na(out)] <- "00"
#       }
#     }
#
#     # match to initial ordering:
...
# }
#
# # flag coicop parents or children:
# has.relative <- function(id, type="child", unbundle=TRUE, direct.only=FALSE){
#
#   # @Args:
#   # id            character vector of coicop ids
#   # type          logical indicating the type of relation,
#   #               either 'child' to search from top to bottom,
#   #               or 'parent' to search from bottom to top
#   # unbundle      logical indicating if coicop bundles (e.g.
#   #               08X, 0531_2) should be taken into account or not
#   # direct.only   logical indicating if only direct children
#   #               should be flagged TRUE (e.g. 03->031) or
...
#
#   # get type:
#   type <- match.arg(arg=type, choices=c("child", "parent"))
#
#   # set to NA if no valid coicop code:
#   id[!hicp::is.coicop(id, unbundle=unbundle)] <- NA_character_
#
...
#     if(type=="parent"){
#
#       if(direct.only) rid <- 1 else rid <- max(nchar(id.tab))-2
#       out <- rep(0, length(id.tab))
#       for(j in 1:rid){
#         out <- out + substr(x=id.tab, start=1, stop=pmax(1, nchar(id.tab)-j))%in%id.tab
#       }
#       out <- out>0  # coerce to logical
#       out[id.tab=="00"] <- FALSE # no parent at top-level
#       if(any(id.tab=="00", na.rm=TRUE)){
#         out[id.tab!="00" & grepl(pattern="^\\d{2}$", x=id.tab)] <- TRUE
#         if(!direct.only){
#           out[id.tab!="00" & out==FALSE] <- TRUE
#         }
#       }
#
#     }
...
#       out <- vector(length=length(id.tab))
#       if(direct.only){
#         for(i in seq_along(id.tab)){
#           id.sub <- id.tab[!id.tab%in%id.tab[i]]
#           id.check <- id.sub[nchar(id.sub)==(nchar(id.tab[i])+1)]
#           out[i] <- any(startsWith(x=id.check, prefix=id.tab[i]), na.rm=TRUE)
#         }
#       }else{
#         for(i in seq_along(id.tab)){
#           id.sub <- id.tab[!id.tab%in%id.tab[i]]
#           out[i] <- any(startsWith(x=id.sub, prefix=id.tab[i]), na.rm=TRUE)
#         }
#       }
#
#       # flag highest-level:
#       id.lvl1 <- formatC(x=1:12, width=2, format="d", flag="0")
#       out[id.tab=="00"&any(id.tab%in%id.lvl1)] <- TRUE
#       if(!direct.only) out[id.tab=="00" & any(id.tab!="00", na.rm=TRUE)] <- TRUE
#
#     }
#
#     # match to initial ordering:
