...
# # old function:
# aggregate <- function(x, w0, w1, grp=NULL, type="laspey", add=list(), keep.lowest=TRUE){
#
#   # @Args:
#   # x             unchained index series, i.e. unchain(x, t)
#   # w0            weights used in the current period
#   # w1            weights from a period t+1
#   # grp           the grouping variable, should be coicop codes
#   #               defined by [0-9]{1,5}; if NULL (default) or
#   #               missing, one aggregation step only
#   # type          the index type, e.g. 'laspey' for the laspeyres index,
#   #               use 'all' to compute all simultaneously
#   # add           named list of ids that can be found in grp, which are
#   #               aggregated
#   # keep.lowest   keep the lowest level indices that form
#   #               the base of all aggregation steps
...
#   .check.num(x=x)
#   .check.num(x=w0, int=c(0,Inf), miss.ok=TRUE)
#   .check.num(x=w1, int=c(0,Inf), miss.ok=TRUE)
#   .check.char(x=grp, null.ok=TRUE)
#   .check.lengths(x=x, y=grp)
#   .check.char(x=type, na.ok=FALSE)
#   .check.log(x=keep.lowest, min.len=1, max.len=1, na.ok=FALSE)
#   if(!is.list(add)) stop("Non-valid input for add -> only lists allowed")
#
#   # input checks on both "w0" and "w1:
#   if(missing(w0) & missing(w1)){stop("Both vectors 'w0' and 'w1' are missing")}
#   if(missing(w0)){w0.miss <- TRUE; w0 <- w1}else{w0.miss <- FALSE}
#   if(missing(w1)){w1.miss <- TRUE; w1 <- w0}else{w1.miss <- FALSE}
#   .check.lengths(x=x, y=w0)
#   .check.lengths(x=x, y=w1)
#
#   # gather "type":
#   type <- tolower(type) # lowercase letters only
#   type_vals <- c("all", "laspey", "paasche", "fisher", "toernq", "walsh") # allowed inputs for "by"
#   type <- match.arg(arg=type, choices=type_vals, several.ok=TRUE)
#   if("all"%in%type){type <- setdiff(x = type_vals, y = "all")}
#
#   if(is.null(grp) || missing(grp)){
#
#     # compute indices:
#     out <- data.frame(
#       "laspey"=laspey(x=x, w0=w0, w1=NULL),
#       "paasche"=paasche(x=x, w0=NULL, w1=w1),
#       "fisher"=fisher(x=x, w0=w0, w1=w1),
#       "toernq"=toernq(x=x, w0=w0, w1=w1),
#       "walsh"=walsh(x=x, w0=w0, w1=w1))
#
#     # subset to one index:
#     out <- as.data.table(out[, type, drop=FALSE])
#
#   }else{
#
#     # gather data:
#     dt <- data.table(grp, w0, w1)
#     dt[, c("laspey","paasche","fisher","toernq","walsh") := x]
#     dt <- dt[grp!="00",]
#     if(!all(is.na(dt$laspey))) dt <- dt[!is.na(laspey), ]
#     setkey(x=dt, key="grp") # do not set the key before
#
#     # data for stepwise aggregation:
#     dt.agg <- copy(dt)
...
#
#       # aggregate to next level:
#       res <- dt.agg[nchar(grp)==i, list(
#         "laspey"=laspey(x=laspey, w0=w0, w1=NULL),
#         "paasche"=paasche(x=paasche, w0=NULL, w1=w1),
#         "fisher"=fisher(x=fisher, w0=w0, w1=w1),
#         "toernq"=toernq(x=toernq, w0=w0, w1=w1),
#         "walsh"=walsh(x=walsh, w0=w0, w1=w1),
#         "w0"=sum(w0, na.rm=TRUE),
#         "w1"=sum(w1, na.rm=TRUE)),
#         by=list("grp"=parent)]
#
#       # # drop NAs to allow that higher-level indices are added in the next step
#       # res <- res[!is.na(laspey),]
...
#
#       # lowest level indices are always laspeyres in HICP:
#       dt.low <- data.table(grp,"is_aggregated"=FALSE,w0,w1)
#       dt.low[, c("laspey","paasche","fisher","toernq","walsh") := x]
#       grp.add <- setdiff(unique(dt.low$grp), unique(out$grp))
#       out <- rbindlist(l=list(out, dt.low[grp%in%grp.add]), use.names=TRUE, fill=TRUE)
#
#       # subset to selected index types:
#       out <- subset(x=out, select=c("grp","is_aggregated","w0","w1", type))
#
#     }else{
#
#       # subset to selected index types:
#       out <- subset(x=out, select=c("grp","w0","w1", type))
#
#     }
#
#     # set names and key:
#     if(w0.miss) out[, "w0":=NULL]
#     if(w1.miss) out[, "w1":=NULL]
#     setkeyv(x=out, cols="grp")
#
#   }
#
